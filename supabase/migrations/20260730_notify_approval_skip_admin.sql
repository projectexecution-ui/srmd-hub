-- Stop emailing the admin (an override role, not a routine approver) on every
-- cost-control approval stage. Real stage approvers (project_head / head /
-- founder) are unaffected. Admin is still notified on a 'returned' event, where
-- it's currently the only matrix recipient. Also keeps the note-in-email
-- behaviour from the previous migration.
...
begin
  ...
  for v_recipient in
    ...
    where p.is_active = true
      ...
      -- Admin is an override, not a routine approver: skip it for cost-control
      -- approval emails (except on a return, where it's the only recipient).
      and not (
