-- Scope Cost Control approval notifications to the people ASSIGNED to the
-- project, not every holder of the approver role.
--
-- Before: notify_on_approval_event picked recipients purely by role — so a
-- budget reaching the Atm Head stage emailed EVERY Atm Head in the org
-- (e.g. Akshay + Hiten got WCE alerts even though only Yash + Amit are
-- assigned to WCE). Meanwhile who can actually *act* was already limited to
-- the named approvers (cc_project_approvers) — so the email fan-out didn't
-- match reality and created noise.
--
-- After: for cost-control working sheets, a role-matched candidate is notified
-- only if they are a named approver for (project, stage-role) in
-- cc_project_approvers — falling back to everyone with the role only when the
-- project has nobody assigned for that stage yet (so nothing is ever missed).
-- This mirrors projectApproverAllows / isWaitingOnMe. Other modules unchanged.
--
-- The full function is reproduced with only the recipient WHERE extended.
...
begin
  ...
  for v_recipient in
    ...
    where p.is_active = true
      ...
      -- Cost Control: notify only people assigned to this project for the
      -- stage's role; role-wide fallback only when nobody is assigned yet.
      and (
        new.module_slug <> 'cost-control'
