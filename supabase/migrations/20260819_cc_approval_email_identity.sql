-- Make the budget-approval EMAIL identity match the Telegram card + the app:
-- add Category (discipline code + name), give the sub-skill its code too, and
-- fall back to the parent project's built-up area for ₹/sft (Common-Expenses
-- sub-projects have none of their own). Only the cost-control block changes;
-- every other module's approval email is byte-for-byte unchanged.
...
begin
  ...
  if new.module_slug = 'cost-control' and new.doc_type = 'cc_working_sheet' then
    ...
    if found then
      ...
      -- ₹/sft area falls back to the parent project (Common-Expenses sub-projects
      -- carry no built-up area of their own).
