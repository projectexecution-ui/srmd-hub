-- Enrich my_approval_inbox with two glance-details for the "Needs you now" home
-- widget: work_label (what the item is FOR — the CC sub-skill / indent
-- sub-project) and raised_by (who raised it). Additive columns; every existing
-- consumer keeps working (extra columns are ignored). DROP first — Postgres
-- won't change a function's return type (added OUT columns) via CREATE OR REPLACE.
drop function if exists public.my_approval_inbox();
