-- Add the notification `type` to the /api/telegram/send payload so the sender
-- can decide image-card (reports/digests) vs text (quick alerts). Everything
-- else in dispatch_telegram_delivery_now is unchanged from 20260812.
...
begin
  ...
  select n.title, n.body, n.url, n.type, np.telegram_chat_id
    into v_title, v_body, v_link, v_type, v_chat
