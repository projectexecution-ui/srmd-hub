-- ============================================================
-- Inventory — returns can be logged by the store's keeper (any base role)
-- ============================================================
-- Full-journey testing surfaced: a warehouse keeper whose base role is 'head'
-- could issue (via the keeper carve-out) but inv_rpc_return_material rejected
-- them ("Unauthorized") because its role gate was role in
-- (store_manager,engineer,admin) only. Authorize the warehouse keeper too, and
-- move the auth check after the line lookup so we know the warehouse. Keeps the
-- is_returnable + outstanding guards.
-- ============================================================
