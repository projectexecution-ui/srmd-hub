  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-4">
      ...
      {projects.length === 0 ? (
        ...
        // Respect the project hierarchy: a parent (e.g. NGH Infra) heads its own
        // band with its buildings nested under it, instead of sitting flat
        // beside them as if it were a sibling.
        const byId = new Map(projects.map(p => [p.id, p]))
        const kids = new Map<string, typeof projects>()
        const roots: typeof projects = []
        for (const p of projects) {
          const par = p.parent_project_id && byId.has(p.parent_project_id) ? p.parent_project_id : null
          if (par) { if (!kids.has(par)) kids.set(par, []); kids.get(par)!.push(p) }
          else roots.push(p)
        }
          ...
          <Link key={p.id} href={`/schedule/${p.id}`} className="block group">
            <Card className="p-4 flex items-center gap-3 group-hover:shadow-md group-hover:ring-1 group-hover:ring-indigo-200 transition">
              {p.item_count > 0 ? (
                ...
                <span className="h-10 w-10 rounded-xl bg-indigo-50 text-indigo-700 grid place-items-center flex-shrink-0"><CalendarClock className="h-5 w-5" /></span>
              )}
              <span className="min-w-0 flex-1">
                <span className="block font-semibold text-gray-900 truncate">{p.code ? `${p.code} — ` : ''}{p.name}</span>
                <span className="mt-0.5 flex items-center gap-2">
                  {p.item_count > 0 ? (
                    <>
                      <span className="text-xs text-gray-500">{p.pct}% done · {p.item_count} item{p.item_count === 1 ? '' : 's'}</span>
                      {p.attention > 0 && <span className="inline-flex items-center rounded-full bg-rose-50 text-rose-700 text-[11px] font-semibold px-2 py-0.5">{p.attention} need action</span>}
                    </>
        ...
        return (
          <div className="space-y-5">
            {roots.map(root => {
              ...
              if (!children.length) return <div key={root.id} className="grid grid-cols-1 sm:grid-cols-2 gap-3">{card(root)}</div>
              const totItems = children.reduce((s, c) => s + c.item_count, 0) + root.item_count
              const totAttn = children.reduce((s, c) => s + c.attention, 0) + root.attention
              return (
                <div key={root.id}>
                  <div className="flex items-center gap-2 mb-2 px-1">
                    <span className="text-[11px] font-bold uppercase tracking-wide text-slate-500">{root.code ?? root.name}</span>
                    <span className="text-[11px] text-slate-400">{root.name} · {children.length} sub-project{children.length === 1 ? '' : 's'} · {totItems} item{totItems === 1 ? '' : 's'}</span>
                    {totAttn > 0 && <span className="inline-flex items-center rounded-full bg-rose-50 text-rose-700 text-[10px] font-bold px-1.5 py-0.5">{totAttn} need action</span>}
                    <span className="flex-1 h-px bg-slate-200" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:pl-4 sm:border-l-2 sm:border-slate-100">
                    {card(root)}
                    {children.map(card)}
                  </div>
                </div>
