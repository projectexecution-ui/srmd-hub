...
export default function PermissionsBoard({ sections, legacy, roles, roleLabels, currentUserIsPortalOwner, canManageRoles, accessInitial }: {
  ...
  const shownLegacy = useMemo(() => ({ ...legacy, rows: filterRows(legacy.rows, q) }), [q, legacy])

  return (
    <div className="space-y-4">
      <div className="relative max-w-xs">
        <Search className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
        <Input value={q} onChange={e => setQ(e.target.value)} placeholder="Search a tab, a pill or a screen…" className="pl-8" />
      </div>

      {shown.length === 0 && shownLegacy.rows.length === 0 ? (
        ...
        <PermissionsMatrix
          ...
          totalModules={sections.flatMap(s => s.rows).filter(r => r.kind === 'module').length + legacy.rows.length}
        />
      )}
    </div>
  )
