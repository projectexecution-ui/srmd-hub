...
export default async function NewWorkingSheetQuickPage({
  ...
  // Full catalogue — the Raise Budget Request dropdowns offer EVERY discipline
  // and sub-skill (not just the project's enabled set), so a newly-added one is
  // immediately usable. Whatever is picked is auto-enabled for the project on
  // submit (cc_ensure_project_scope) — no separate per-project setup step.
  const [dRes, sRes] = await Promise.all([
    supabase.from('cc_disciplines').select('id, code, name').eq('is_archived', false).order('display_order'),
    supabase.from('cc_sub_skills').select('id, discipline_id, code, name').eq('is_archived', false).order('code'),
  ])
  type DRow = { id: string; code: string; name: string }
  type SRow = { id: string; discipline_id: string; code: string; name: string }
  const allDisciplines = (dRes.data ?? []) as DRow[]
  const allSubSkills = (sRes.data ?? []) as SRow[]
