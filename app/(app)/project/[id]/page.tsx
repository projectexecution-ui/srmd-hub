...
/** Overview — the project's money in the Internal Estimate's own colours, then
 *  a plain list of what this project holds. Kept to a summary on purpose: the
 *  full seven-column table lives one click away on Budget. */
export default async function ProjectOverviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-5">
      {/* Setup nudge — 23 of 39 projects have no area, so this is common. */}
      {(project.setupPct < 100 || sft === 0) && (
        <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <Link
            href={`/cost-control/projects/${id}/setup`}
      ...
      {/* Money — same order and same colours as the live Internal Estimate. */}
      <section className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        <header className="px-4 py-2.5 border-b border-gray-100 flex items-center justify-between gap-2 flex-wrap">
          ...
          <Link href={tabHref(id, PROJECT_TABS[1])} className="text-xs font-medium text-indigo-700 hover:underline">
            Open the full table →
          </Link>
        </header>
      ...
      {/* What this project holds — honest about what is real. */}
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          {PROJECT_TABS.filter(t => t.slug !== '').map(tab => (
            <Link
              key={tab.slug}
              href={tabHref(id, tab)}
              className={[
