...
export default async function InventoryLandingPage() {
  ...
  // Role-based section tiles
  const sections: Array<{
    href: string; title: string; sub: string;
    icon: React.ComponentType<{ className?: string }>;
    show: boolean; comingSoon?: boolean;
  }> = [
    {
...
function SectionTile({
  href, title, sub, icon: Icon, comingSoon,
}: {
  href: string; title: string; sub: string;
  icon: React.ComponentType<{ className?: string }>; comingSoon?: boolean;
}) {
  const inner = (
    <Card className="relative p-4 md:p-5 h-full hover:shadow-md hover:-translate-y-0.5 transition-all">
      ...
      {comingSoon && (
        <span className="absolute top-3 right-3 text-[10px] uppercase tracking-wide font-bold text-gray-400">
          Soon
        </span>
      )}
    </Card>
