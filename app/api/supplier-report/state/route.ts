// Server-backed state for the Supplier Report module — mirrors
// /api/contractor-report/state. GET returns the org-wide blob; PUT writes a
// new version and snapshots the previous one. The report page loads this on
...
export async function PUT(req: Request) {
  ...
  const { error: snapErr } = await supabase
    .from('supplier_report_state_history')
    .insert({ state_id: STATE_ID, state: current.state, version: current.version, snapshot_by: user.id })
  if (snapErr) console.warn('[supplier-report] snapshot failed:', snapErr.message)
