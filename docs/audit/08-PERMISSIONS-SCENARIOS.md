...
- **"Screens and powers"** — the 13 modules the revamp stands on. **"Old screens — not in the revamp"** — the other 18, folded away at the bottom (still routable by URL, so their switches still matter; a search unfolds them).
...
| 14 | Search "BOQ upload" | Shows the pill with its tab (and the module whose hint names the tab). Old screens unfold if they match. | ✅ tested |
| 15 | 74 rows is a lot | Pills are collapsed under their tab by default; "Show every pill" / "Tabs only" buttons; old screens folded. | ✅ |
...
| 32 | Which modules count as "in the revamp" | `REVAMP_MODULES` in `lib/revamp/permissions.ts` — the lanes in `nav.ts` plus every module a tab gates on plus admin. Anything else is old. Adding a lane means adding it there. | ✅ single list |
