...
const DESAI: WoCertRow[] = [
  ...
  wo({ certificate_id: 3, invoice_no: 'SRASSK-GHB/10', invoice_date: '2026-02-02', gross_bill_amt: 3639030, recoveries: 736350, paid_amt: 0, outstanding_amt: 2902680 }), // IN4's duplicate
...
const SUP: SupCertRow[] = [
  { certificate_id: 501, kind: 'payment', certificate_no: '1187', status: 1, supplier_id: 5, supplier_name: 'Ambuja Cement', po_id: 3301, category: 'Cement', certified_amt: 120000, landed_cost: 141600, tax_deduction: 0, adv_recovery: 0, debit_note_adj: 0, retention: 0, payable: 141600, paid: 141600, outstanding: 0 },
]
...
describe('payments: what money left, folded to the truth', () => {
  it('lists only money out, newest first, with IN4 dates until the Trust confirms', () => {
    ...
    // Supplier payments carry no IN4 date at all.
    expect(book.payments.at(-1)!.date).toBeNull()
    expect(book.totals.undatedPaid).toBe(141600)
  })

  it('folds IN4 duplicate bills to one, keeps the paid row, and says so', () => {
    const book = buildPayments(DESAI, [], [])
    expect(book.duplicates).toHaveLength(1)
    expect(book.duplicates[0].kept.id).toBe('WO-2')
    expect(book.duplicates[0].dropped.map(d => d.id)).toEqual(['WO-3'])
    expect(book.bills.map(b => b.id)).not.toContain('WO-3')
    // The raw toggle shows IN4 as it is.
    expect(buildPayments(DESAI, [], [], { raw: true }).bills.map(b => b.id)).toContain('WO-3')
  })
...
describe('party ledger in Tally format', () => {
  ...
  it('credits the bill, debits retention, recoveries and the payment, and returns the balance to what was owed', () => {
    const l = buildPartyLedger(book.bills, 'Desai Construction Pvt Ltd.', '2026-27', book.duplicates.length)
