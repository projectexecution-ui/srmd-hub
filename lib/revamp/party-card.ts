...
export async function loadPartyCard(kind: 'contractor' | 'supplier', id: number): Promise<({ card: PartyCard | null }) & In4Read> {
  ...
  if (!data) return { card: null, in4: 'mirror' }
  ...
  try {
    if (kind === 'contractor') {
      const { data, error } = await supabase.rpc('in4_party_wo_orders', { p_contractor_id: id })
      if (error) return { card: empty, in4: 'unavailable', in4Error: error.message }
      const orders: PartyOrder[] = ((data ?? []) as Array<Record<string, unknown>>).map(r => ({
        id: n(r.wo_id), ref: s(r.wo_no) ?? `WO ${r.wo_id}`, date: iso(r.wo_dt), project: s(r.project), category: s(r.category)?.replace(/^\d+\s+/, '') ?? null,
        status: s(r.status), gross: n(r.gross), paid: n(r.paid), retention: n(r.retention), grnStatus: null, firstGrn: null,
      }))
      return { card: { ...empty, orders, totals: totalsOf(orders) }, in4: 'mirror' }
    }
    const [poRes, priceRes] = await Promise.all([
      supabase.rpc('in4_party_po_orders', { p_supplier_id: id }),
      supabase.rpc('in4_party_price_gaps', { p_supplier_id: id, p_limit: 25 }),
    ])
    if (poRes.error) return { card: empty, in4: 'unavailable', in4Error: poRes.error.message }
    const prices = (priceRes.data ?? []) as Array<Record<string, unknown>>
    const orders: PartyOrder[] = ((poRes.data ?? []) as Array<Record<string, unknown>>).map(r => ({
