...
/** One repeating block of a template: the tags that make up its row, the
 *  rows to write, and what to say when there are none. */
export interface RowGroup {
  tags: readonly string[]
  rows: PrintRow[]
  /** Printed as a single full-width row when `rows` is empty. */
  emptyText: string
  /** Values are IN4's own html and go in unescaped (the PO conditions,
   *  which carry line breaks). Never anything a CT Hub user typed. */
  raw?: boolean
}
...
export function renderTemplate(
  ...
  return renderTemplateGroups(templateHtml, scalars, [
    // An order with NO lines does not get an empty table frame. 378 of IN4's
    // 2,157 work orders hold no BOQ breakdown at all, and a header row over
    // blank space reads as a rendering failure rather than as the truth about
    // that order. One stated row says it.
    { tags: rowTags, rows, emptyText: 'IN4 holds no BOQ breakdown for this work order.' },
  ], rawTags)
}

/**
 * The same, for a template with SEVERAL repeating rows. IN4's purchase-order
 * template has three — the material lines, the delivery schedule and the
 * conditions — each its own `<tr>` of tags. `rows` in the result counts the
 * first group (the document's line items); `groupRows` counts each.
 */
export function renderTemplateGroups(
  templateHtml: string,
  scalars: Record<string, string | null | undefined>,
  groups: readonly RowGroup[],
  rawTags: readonly string[] = [],
): RenderResult & { groupRows: number[] } {
  let html = templateHtml
  const asked = new Set(html.match(/\[[A-Za-z0-9_.\- ]+\]/g) ?? [])

  // ORDER MATTERS. The rows are cut out FIRST and put back LAST, so nothing
  // inside them is ever scanned for tags. IN4's own BOQ descriptions contain
  // square brackets — the 423-line order WO/SRET/RU/2024-25/80 has [ARMOURED]
  // in nineteen of its lines — and with the rows injected before the
  // substitution pass, a description reading [Project] would have been
  // silently replaced by the project name.
  const cut: { token: string; rowTemplate: string; group: RowGroup }[] = []
  groups.forEach((group, i) => {
    const anchor = group.tags.find(t => html.includes(t))
    if (!anchor) return
    const at = html.indexOf(anchor)
    const open = html.lastIndexOf('<tr', at)
    const closeAt = html.indexOf('</tr>', at)
    if (open === -1 || closeAt === -1) return
    const token = ` CT_ROWS_${i} `
    cut.push({ token, rowTemplate: html.slice(open, closeAt + '</tr>'.length), group })
    html = html.slice(0, open) + token + html.slice(closeAt + '</tr>'.length)
  })
  const cutTags = new Set(cut.flatMap(c => [...c.group.tags]))
  ...
  for (const tag of asked) {
    ...
    // A row tag is consumed by the block below. If that block could not be
    // found — IN4 restructuring the template, say — the tag would otherwise
    // PRINT RAW on the page. Clear and report it instead.
    if (cutTags.has(tag)) continue
    unresolved.push(tag)
    html = html.split(tag).join('')
  }

  // ── The rows, put back last ──────────────────────────────────────────────
  const groupRows = groups.map(() => 0)
  cut.forEach(({ token, rowTemplate, group }) => {
    const rendered = group.rows.map(r =>
      group.tags.reduce((acc, tag) => acc.split(tag).join(group.raw ? (r[tag] ?? '') : escapeHtml(r[tag] ?? '')), rowTemplate),
    ).join('\n')
    const cells = (rowTemplate.match(/<t[dh][\s>]/gi) ?? []).length || 1
    const block = group.rows.length > 0 ? rendered
      : `<tr><td colspan="${cells}" style="text-align:center; padding:8px; color:#555">`
        + escapeHtml(group.emptyText) + '</td></tr>'
    html = html.split(token).join(block)
    groupRows[groups.indexOf(group)] = group.rows.length
  })
  return { html, unresolved: unresolved.sort(), rows: groupRows[0] ?? 0, groupRows }
}
...
/** Where each part of the printed order came from, and what IN4 does not
 *  hold. Shown on screen before printing so nobody sends a contract out
 *  without noticing it has no terms on it. */
export interface WoSources {
  /** 'print' = IN4's own printed BOQ block; 'ordered' = the ordered-BOQ
   *  fact, used when the print block is empty; 'none' = neither. */
  boq: 'print' | 'ordered' | 'none'
  boqLines: number
  conditions: number
  /** Plain-English gaps, each one a fact about IN4 rather than a warning
   *  about the renderer. */
  gaps: string[]
}
...
export function pct(v: unknown): string | null {
...
export function fmtNum(v: unknown, dp = 3): string | null {
  const n = v == null ? NaN : Number(v)
  if (!Number.isFinite(n)) return null
...
export async function loadWoPrint(woId: number): Promise<WoPrintData> {
  ...
  // 498 of those DO have their ordered BOQ in the warehouse fact. Falling
  // back recovers the item table on well over a fifth of all orders that
  // would otherwise print with no breakdown. The fallback is named on the
  // page, because it is IN4's ordered BOQ rather than the block IN4 itself
  // would have printed.
  if (boq.length === 0) {
    boq = await in4Query<Record<string, unknown>>(`
      SELECT ROW_NUMBER() OVER (ORDER BY d.BOQ_NAME, f.ITEM_ID) SLNO,
             COALESCE(NULLIF(d.BOQ_SUBNAME,''), d.BOQ_NAME) NAME,
             d.BOQ_DESCRIPTION DESCRIPTION, d.UOM UNIT,
             f.QUANTITY, f.RATE, f.AMT AMOUNT
      FROM BI.FACT_ENGG_WORK_ORDER_BOQ f
      -- ITEM_ID, never BOQ_ID: the two facts disagree on BOQ_ID by one, so a
      -- BOQ_ID join silently returns nothing (see lib/in4/boq.ts).
  ...
  const gaps: string[] = []
  if (boqSource === 'none') gaps.push('IN4 holds no BOQ breakdown for this order.')
  if (boqSource === 'ordered') gaps.push('The item table is IN4\'s ordered BOQ. IN4\'s own printed BOQ block is empty for this order.')
  if (conds.length === 0) {
    // 470 of 2,157 orders. Worth saying loudly: this is a contract going
    // out with no conditions attached to it.
    gaps.push('This order has NO terms and conditions in IN4. The contract will print without any.')
  }
  if (wo.WO_GROSS_VALUE == null || Number(wo.WO_GROSS_VALUE) === 0) {
    gaps.push('IN4 holds no gross value for this order, so that line is blank.')
  }
  if (!str(wo.co_gstin)) {
    // SRASSK holds no GSTIN anywhere in IN4 — not on the company, not on a
    // project, not on a sub-project — and it is the paying company on 1,017
    // of the work orders. That is a gap to fill in IN4, never to invent.
    gaps.push('IN4 holds no GST number for the paying trust, so the GST line is blank.')
  }

  return {
    woId,
    displayNo,
    sources: { boq: boqSource, boqLines: boq.length, conditions: conds.length, gaps },
    scalars,
    rows,
...
export function wrapForPrint(
  body: string,
  meta: {
    displayNo: string; templateName: string; unresolved: string[]; rows: number
    /** What IN4 does and does not hold for this order. `boq` is the work
     *  order's item-table provenance; a purchase order leaves it out. */
    sources?: { boq?: 'print' | 'ordered' | 'none'; boqLines?: number; conditions: number; gaps: string[] }
    /** What one repeating row is called on the foot line: "BOQ line" for a
     *  work order, "item" for a purchase order. */
    noun?: string
  },
): string {
...
<style>
  ...
  * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
  ...
  /* Screen only. This is a check on IN4's data before the document leaves the
     building; it is not part of the contract and must never print. */
  .ct-check { margin: 8px 0; padding: 8px 12px; border: 1px solid #fcd34d;
              background: #fffbeb; border-radius: 6px;
              font: 12px/1.5 system-ui, sans-serif; color: #78350f; }
  .ct-check b { display: block; margin-bottom: 2px; }
  .ct-check ul { margin: 0; padding-left: 18px; }
  ...
  @media print { .ct-bar, .ct-check { display: none } body { font-size: 10px } }
</style></head>
<body>
  <div class="ct-bar">
  ...
  ${meta.sources && meta.sources.gaps.length > 0 ? `
  <div class="ct-check">
    <b>Before you send this out</b>
    <ul>${meta.sources.gaps.map(g => `<li>${g}</li>`).join('')}</ul>
  </div>` : ''}
  <div class="ct-doc">${body}</div>
  <div class="ct-foot">
    Rendered by CT Hub from IN4 on ${stamp} · ${meta.rows} ${meta.noun ?? 'BOQ line'}${meta.rows === 1 ? '' : 's'}${
      meta.sources?.boq === 'ordered' ? ' (from IN4\x27s ordered BOQ)' : ''
    }${meta.sources ? ` · ${meta.sources.conditions} condition${meta.sources.conditions === 1 ? '' : 's'}` : ''}.
    IN4 remains the system of record for this order.
    ${meta.unresolved.length > 0
      ? `<br><span class="ct-warn">Blank on this copy — IN4 holds nothing CT Hub could put there: ${meta.unresolved.join(', ')}</span>`
      : ''}
  </div>
</body></html>`
}
