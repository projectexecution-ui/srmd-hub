...
// IN4 holds a name for every one of the 5,080 purchase-order lines.
const materials = new Map([[501, 'PU SHAPE DRAIN T-6 450 X 450'], [502, 'PU SHAPE DRAIN T-6 LID 450 X 450'], [503, 'CPVC PIPE 20MM']])
...
const item = (o: Partial<Parameters<typeof buildPoList>[1][number]> = {}) => ({
  po_id: 1257, subproject_id: 71, material_id: 501, material_value: 2_981_200, ...o,
})
...
describe('purchase-order picker', () => {
  it('fills supplier, ordered value, billed and balance from IN4', () => {
    const [p] = buildPoList([po()], [item()], [cert()], suppliers, materials)
  ...
  it('reads the trust out of the PO number, the same way a WO number is read', () => {
    expect(buildPoList([po()], [], [], suppliers, materials)[0].trust).toBe('SRASSK')
    expect(buildPoList([po({ po_no: 'PO/SRET/RU/2025-26/299' })], [], [], suppliers, materials)[0].trust).toBe('SRET')
  })

  // IN4 leaves the PO header sub-project empty on all 1,450 orders and fills
  // it on every line, so the order genuinely names one — one level down.
  it('books to the sub-project its lines put the most money into', () => {
    ...
    expect(p.subprojectIds).toEqual([7, 71])
  })

  it('offers every sub-project it spans rather than booking one quietly', () => {
    expect(buildPoList([po()], [item()], [], suppliers, materials)[0].subprojectIds).toEqual([71])
    ...
    expect(spread.subprojectIds).toEqual([5, 7, 71])
  })

  it('leaves the sub-project empty rather than guessing when no line names one', () => {
    const [p] = buildPoList([po()], [item({ subproject_id: null })], [], suppliers, materials)
    expect(p.subprojectId).toBeNull()
    expect(p.subprojectIds).toEqual([])
  })
  ...
  it('carries no category at all on an order nothing has been billed against', () => {
    expect(buildPoList([po()], [item()], [], suppliers, materials)[0].categoryName).toBeNull()
  })
  ...
  it('derives the retention rate from what IN4 actually deducted', () => {
    ...
    expect(buildPoList([po()], [item()], [cert()], suppliers, materials)[0].retentionPct).toBe(0)
    expect(buildPoList([po()], [item()], [cert({ retention: 50_000 })], suppliers, materials)[0].retentionPct).toBe(5)
    expect(buildPoList([po()], [item()], [], suppliers, materials)[0].retentionPct).toBeNull()
  })
  ...
  it('never reports a negative balance on an over-billed order', () => {
    const [p] = buildPoList([po({ po_value: 1000 })], [item()], [cert({ landed_cost: 1500 })], suppliers, materials)
  ...
  it('leaves the supplier blank rather than guessing when the party is unknown', () => {
    expect(buildPoList([po({ supplier_id: 999 })], [], [], suppliers, materials)[0].party).toBe('')
  })

  // Aksha, 15 Sep 2026: "PO comes with Material name and Qty etc". It does —
  // all 5,080 lines — so the scope is read off the order like everything else
  // rather than being typed in.
  it('reads the scope off the material lines, biggest first', () => {
    const [p] = buildPoList([po()], [
      item({ material_id: 503, material_value: 10_000 }),
      item({ material_id: 501, material_value: 900_000 }),
    ], [], suppliers, materials)
    expect(p.workDescription).toBe('PU SHAPE DRAIN T-6 450 X 450, CPVC PIPE 20MM')
  })

  it('leaves the scope empty when IN4 names no material, rather than inventing', () => {
    expect(buildPoList([po()], [item({ material_id: null })], [], suppliers, materials)[0].workDescription).toBeNull()
    expect(buildPoList([po()], [item({ material_id: 99999 })], [], suppliers, materials)[0].workDescription).toBeNull()
  })
})
