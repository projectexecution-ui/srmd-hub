    ...
    <div className="rounded-xl border border-gray-200 bg-white">
      <div className="px-4 py-2.5 border-b border-gray-100">
        <h3 className="text-sm font-bold text-gray-900 inline-flex items-center gap-2">
          ...
          {comments.length > 0 && <span className="text-[11px] font-semibold text-gray-500 bg-gray-100 rounded-full px-1.5 py-0.5">{comments.length}</span>}
        </h3>
      </div>

      <div className="p-4 space-y-3">
        {/* The latest two in the open; anything older folds behind one line
            (Aksha, 23 Sep 2026 — declutter). Still newest last, so the thread
            reads top-to-bottom into the compose box. */}
        {comments.length === 0 ? (
          <p className="text-sm text-gray-400">No comments yet.</p>
        ) : (() => {
          const earlier = comments.slice(0, Math.max(0, comments.length - 2))
          const latest = comments.slice(-2)
          const item = (c: CommentRow) => {
            ...
            <>
              {earlier.length > 0 && (
                <details className="group">
                  <summary className="cursor-pointer select-none text-xs font-semibold text-indigo-700 hover:underline list-none">
                    <span className="group-open:hidden">{earlier.length} earlier comment{earlier.length === 1 ? '' : 's'}</span>
                    <span className="hidden group-open:inline">Hide earlier comments</span>
                  </summary>
                  <ul className="space-y-3 mt-3">{earlier.map(item)}</ul>
                </details>
              )}
              <ul className="space-y-3">{latest.map(item)}</ul>
            </>
          )
        })()}
